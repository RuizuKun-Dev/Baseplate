...
function VRNavigation:ControlCharacterGamepad(
    actionName,
    inputState,
    inputObject
)
    ...
    if self.moving and self.currentPoints then
        ...
        if moveDist < POINT_REACHED_THRESHOLD then
            ...
            if obstructPart then
                ...
                local jumpCheckRay = Ray.new(obstructPoint + moveDir * 0.5 + heightOffset, 
-heightOffset)
...
function VRNavigation:Enable(enable)
    ...
    if enable then
        self.navigationRequestedConn = VRService.NavigationRequested:Connect(function(
            destinationCFrame,
            inputUserCFrame
        )
        ...
        ContextActionService:BindAction('MoveThumbstick', (function(
            actionName,
            inputState,
            inputObject
        )
