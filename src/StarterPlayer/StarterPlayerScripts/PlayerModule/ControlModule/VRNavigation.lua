...
function VRNavigation:OnHeartbeat(dt)
	...
	if self.moving and self.currentPoints then
		...
		if moveDist < POINT_REACHED_THRESHOLD then
			...
			local obstructPart, obstructPoint, obstructNormal = workspace:FindPartOnRayWithIgnoreList(
				obstructRay,
				ignoreTable
			)
			...
			if obstructPart then
				...
				local jumpCheckPart, jumpCheckPoint, jumpCheckNormal = workspace:FindPartOnRayWithIgnoreList(
					jumpCheckRay,
					ignoreTable
				)
