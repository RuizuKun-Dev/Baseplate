...
local FFlagUserExcludeNonCollidableForPathfinding = FFlagUserExcludeNonCollidableForPathfindingSuccess
	and FFlagUserExcludeNonCollidableForPathfindingResult
...
local function UpdateIgnoreTag(newIgnoreTag)
	...
	if CurrentIgnoreTag ~= nil then
		...
		TaggedInstanceAddedConnection = CollectionService
			:GetInstanceAddedSignal(CurrentIgnoreTag)
			:Connect(function(ignorePart)
				table.insert(CurrentIgnoreList, ignorePart)
			end)
		TaggedInstanceRemovedConnection = CollectionService
			:GetInstanceRemovedSignal(CurrentIgnoreTag)
			:Connect(function(ignorePart)
...
local function Pather(endPoint, surfaceNormal, overrideUseDirectPath)
			...
			if this.AgentCanFollowPath then
				if this.DirectPath then
					this.pointList = {
						...
						PathWaypoint.new(
							this.TargetPoint,
							this.DirectPathRiseFirst and Enum.PathWaypointAction.Jump or Enum.PathWaypointAction.Walk
						),
					}
	...
	function this:MoveToNextWayPoint(currentWaypoint, nextWaypoint, nextWaypointIdx)
		...
		this.CurrentWaypointPlaneNormal = Vector3.new(
			this.CurrentWaypointPlaneNormal.X,
			0,
			this.CurrentWaypointPlaneNormal.Z
		)
