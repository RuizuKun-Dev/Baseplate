local FFlagUserExcludeNonCollidableForPathfindingSuccess, FFlagUserExcludeNonCollidableForPathfindingResult = pcall(function(
)
...
local function UpdateIgnoreTag(newIgnoreTag)
    ...
    if CurrentIgnoreTag ~= nil then
        ...
        TaggedInstanceAddedConnection = CollectionService:GetInstanceAddedSignal(CurrentIgnoreTag):Connect(function(
            ignorePart
        )
            table.insert(CurrentIgnoreList, ignorePart)
        end)
        TaggedInstanceRemovedConnection = CollectionService:GetInstanceRemovedSignal(CurrentIgnoreTag):Connect(function(
            ignorePart
        )
...
local function Pather(endPoint, surfaceNormal, overrideUseDirectPath)
    ...
    function this:MoveToNextWayPoint(
        currentWaypoint,
        nextWaypoint,
        nextWaypointIdx
    )
