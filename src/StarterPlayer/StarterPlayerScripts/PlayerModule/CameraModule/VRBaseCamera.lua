...
function VRBaseCamera:OnEnable(enable)
    if enable then
...
function VRBaseCamera:GetCameraToSubjectDistance()
    return self.currentSubjectDistance
end
function VRBaseCamera:SetCameraToSubjectDistance(desiredSubjectDistance)
    local lastSubjectDistance = self.currentSubjectDistance
    local newSubjectDistance = math.clamp(desiredSubjectDistance, 0, player.CameraMaxZoomDistance)

    if newSubjectDistance < 1 then
...
function VRBaseCamera:GetVRFocus(subjectPosition, timeDelta)
    ...
    self.cameraTranslationConstraints = Vector3.new(self.cameraTranslationConstraints.x, math.min(1, self.cameraTranslationConstraints.y + timeDelta), self.cameraTranslationConstraints.z)

    local cameraHeightDelta = Vector3.new(0, self:GetCameraHeight(), 0)
    local newFocus = CFrame.new(Vector3.new(subjectPosition.x, lastFocus.y, subjectPosition.z):lerp(subjectPosition + cameraHeightDelta, self.cameraTranslationConstraints.y))
...
function VRBaseCamera:UpdateFadeFromBlack(timeDelta)
    local VRFade = Lighting:FindFirstChild('VRFade')
    ...
    if VRBlur then
        if self.VREdgeBlurTimer > 0 then
            ...
            if VRScreen then
                ...
                if VRBlur then
                    VRBlur.ImageTransparency = 1 - math.clamp(self.VREdgeBlurTimer, 0.01, VR_SCREEN_EGDE_BLEND_TIME) * (1 / VR_SCREEN_EGDE_BLEND_TIME)
                end
            end
        else
...
function VRBaseCamera:GetSubjectCFrame()
...
function VRBaseCamera:GetSubjectPosition()
