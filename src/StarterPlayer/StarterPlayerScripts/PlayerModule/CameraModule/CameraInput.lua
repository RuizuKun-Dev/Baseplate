...
do
    ...
    function CameraInput.getRotation(disableKeyboardRotation)
        ...
        local result = kKeyboard * ROTATION_SPEED_KEYS + kGamepad * ROTATION_SPEED_GAMEPAD + kMouse * ROTATION_SPEED_MOUSE + kPointerAction * ROTATION_SPEED_POINTERACTION + kTouch * ROTATION_SPEED_TOUCH
    ...
    do
        local function thumbstick(action, state, input)
            ...
            gamepadState[input.KeyCode.Name] = Vector2.new(thumbstickCurve(position.X), 
-thumbstickCurve(position.Y))
        ...
        function CameraInput.setInputEnabled(_inputEnabled)
            ...
            if inputEnabled then
                ContextActionService:BindActionAtPriority('RbxCameraThumbstick', thumbstick, false, CAMERA_INPUT_PRIORITY, Enum.KeyCode.Thumbstick2)
                ContextActionService:BindActionAtPriority('RbxCameraKeypress', keypress, false, CAMERA_INPUT_PRIORITY, Enum.KeyCode.Left, Enum.KeyCode.Right, Enum.KeyCode.I, Enum.KeyCode.O)
