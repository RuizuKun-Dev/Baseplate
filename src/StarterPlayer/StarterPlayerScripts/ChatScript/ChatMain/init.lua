...
spawn(function()
    ...
    local animationFps = ChatSettings.ChatAnimationFPS or 20
    local updateWaitTime = 1 / animationFps
    local lastTick = tick()
...
function CheckIfPointIsInSquare(checkPos, topLeft, bottomRight)
    return(topLeft.X <= checkPos.X and checkPos.X <= bottomRight.X and topLeft.Y <= checkPos.Y and checkPos.Y <= bottomRight.Y)
end
...
function UpdateMousePosition(mousePos)
    if not (moduleApiTable.Visible and moduleApiTable.IsCoreGuiEnabled and (moduleApiTable.TopbarEnabled or ChatSettings.ChatOnWithTopBarOff)) then
        return
    end
...
do
    function Util.Signal()
        ...
        function sig:wait()
            ...
            assert(mArgData, 
[[Missing arg data, likely due to :TweenSize/Position corrupting threadrefs.]])
...
EventFolder.OnNewMessage.OnClientEvent:connect(function(
    messageData,
    channelName
)
    local channelObj = ChatWindow:GetChannel(channelName)
...
EventFolder.OnNewSystemMessage.OnClientEvent:connect(function(
    messageData,
    channelName
)
...
EventFolder.OnChannelJoined.OnClientEvent:connect(function(
    channel,
    welcomeMessage,
    messageLog,
    channelNameColor
...
EventFolder.OnMuted.OnClientEvent:connect(function(channel) end)
EventFolder.OnUnmuted.OnClientEvent:connect(function(channel) end)
...
coroutine.wrap(function()
    ...
    if ChannelNameColorUpdated then
        ChannelNameColorUpdated.OnClientEvent:connect(function(
            channelName,
            channelNameColor
        )
...
if PlayerBlockedEvent then
    PlayerBlockedEvent.Event:connect(function(player)
        if MutePlayer(player) then
            ...
            SendSystemMessageToSelf(ChatLocalization:Get('GameChat_ChatMain_SpeakerHasBeenBlocked', string.format("Speaker '%s' has been blocked.", playerName), {RBX_NAME = playerName}))
        end
    end)
end
if PlayerMutedEvent then
    PlayerMutedEvent.Event:connect(function(player)
        if MutePlayer(player) then
            ...
            SendSystemMessageToSelf(ChatLocalization:Get('GameChat_ChatMain_SpeakerHasBeenMuted', string.format("Speaker '%s' has been muted.", playerName), {RBX_NAME = playerName}))
        end
    end)
end
...
if PlayerUnBlockedEvent then
    PlayerUnBlockedEvent.Event:connect(function(player)
        if UnmutePlayer(player) then
            ...
            SendSystemMessageToSelf(ChatLocalization:Get('GameChat_ChatMain_SpeakerHasBeenUnBlocked', string.format("Speaker '%s' has been unblocked.", playerName), {RBX_NAME = playerName}))
        end
    end)
end
if PlayerUnMutedEvent then
    PlayerUnMutedEvent.Event:connect(function(player)
        if UnmutePlayer(player) then
            ...
            SendSystemMessageToSelf(ChatLocalization:Get('GameChat_ChatMain_SpeakerHasBeenUnMuted', string.format("Speaker '%s' has been unmuted.", playerName), {RBX_NAME = playerName}))
        end
    end)
end
