...
local function lerpLength(msg, min, max)
    return min + (max - min) * math.min(getMessageLength(msg) / 75, 1)
end
...
function createChatBubbleMain(filePrefix, sliceRect)
    ...
    chatBubbleMain.Size = UDim2.new(1, 0, 1, 0)
...
function createScaledChatBubbleWithTail(
    filePrefix,
    frameScaleSize,
    position,
    sliceRect
...
local function initChatBubbleType(
    chatBubbleType,
    fileName,
    imposterFileName,
    isInset,
    ...
    this.ChatBubbleWithTail[chatBubbleType] = createChatBubbleWithTail(fileName, UDim2.new(0.5, 
-CHAT_BUBBLE_TAIL_HEIGHT, 1, isInset and -1 or 0), UDim2.new(0, 30, 0, CHAT_BUBBLE_TAIL_HEIGHT), sliceRect)
    this.ScalingChatBubbleWithTail[chatBubbleType] = createScaledChatBubbleWithTail(fileName, 0.5, UDim2.new(
-0.5, 0, 0, isInset and -1 or 0), sliceRect)
end
...
function this:UpdateChatLinesForOrigin(origin, currentBubbleYPos)
    ...
    for index=(#bubbleQueueData - 1), 1, -1 do
        ...
        local udimValue = UDim2.new(bubble.Position.X.Scale, bubble.Position.X.Offset, 1, currentBubbleYPos - bubble.Size.Y.Offset - CHAT_BUBBLE_TAIL_HEIGHT)
...
function this:CreateChatLineRender(
    instance,
    line,
    onlyCharacter,
    fifo,
    ...
    if billboardGui then
        ...
        if UserFixBubbleChatText then
            ...
            chatBubbleRender:TweenSizeAndPosition(UDim2.fromOffset(width, height), UDim2.new(0.5, 
-width / 2, 1, -height), Enum.EasingDirection.Out, Enum.EasingStyle.Elastic, 0.1, true, function(
            )
            ...
            chatBubbleRender:TweenSizeAndPosition(UDim2.new(bubbleWidthScale, 0, 0, newChatBubbleOffsetSizeY), UDim2.new((1 - bubbleWidthScale) / 2, 0, 1, 
-newChatBubbleOffsetSizeY), Enum.EasingDirection.Out, Enum.EasingStyle.Elastic, 0.1, true, function(
            )
                bubbleText.Visible = true
            end)
            this:SetBillboardGuiLOD(billboardGui, line.Origin)
