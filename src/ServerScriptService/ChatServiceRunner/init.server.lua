...
EventFolder.SayMessageRequest.OnServerEvent:connect(function(
    playerObj,
    message,
    channel
)
...
EventFolder.MutePlayerRequest.OnServerInvoke = function(
    playerObj,
    muteSpeakerName
)
...
EventFolder.UnMutePlayerRequest.OnServerInvoke = function(
    playerObj,
    unmuteSpeakerName
)
...
EventFolder.SetBlockedUserIdsRequest.OnServerEvent:Connect(function(
    player,
    blockedUserIdsList
)
        ...
        if (channel) then
            if (channel.Joinable) then
                if (not speaker:IsInChannel(channel.Name)) then
                    ...
                    local msg = ChatLocalization:FormatMessageToSend('GameChat_SwitchChannel_NowInChannel', string.format("You are now chatting in channel: '%s'", channel.Name), 'RBX_NAME', channel.Name)

                    speaker:SendSystemMessage(msg, channel.Name)
                end
            else
                local msg = ChatLocalization:FormatMessageToSend('GameChat_ChatServiceRunner_YouCannotJoinChannel', "You cannot join channel '" .. channelName .. "'.", 'RBX_NAME', channelName)

                speaker:SendSystemMessage(msg, fromChannelName)
            end
        else
            local msg = ChatLocalization:FormatMessageToSend('GameChat_ChatServiceRunner_ChannelDoesNotExist', "Channel '" .. channelName .. "' does not exist.", 'RBX_NAME', channelName)
    ...
    if (speaker) then
        if (speaker:IsInChannel(channelName)) then
            if (channel.Leavable) then
                ...
                local msg = ChatLocalization:FormatMessageToSend('GameChat_ChatService_YouHaveLeftChannel', string.format("You have left channel '%s'", channelName), 'RBX_NAME', channel.Name)

                speaker:SendSystemMessage(msg, 'System')
            else
                local msg = ChatLocalization:FormatMessageToSend('GameChat_ChatServiceRunner_YouCannotLeaveChannel', ("You cannot leave channel '" .. channelName .. "'."), 'RBX_NAME', channelName)

                speaker:SendSystemMessage(msg, fromChannelName)
            end
        else
            local msg = ChatLocalization:FormatMessageToSend('GameChat_ChatServiceRunner_YouAreNotInChannel', ("You are not in channel '" .. channelName .. "'."), 'RBX_NAME', channelName)
...
ChatService:RegisterProcessCommandsFunction('default_commands', function(
    fromSpeaker,
    message,
    channel
)
...
systemChannel.WelcomeMessage = ChatLocalization:FormatMessageToSend('GameChat_ChatServiceRunner_SystemChannelWelcomeMessage', 'This channel is for system and game notifications.')
