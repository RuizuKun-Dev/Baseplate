...
function methods:SendSystemMessageToSpeaker(message, speakerName, extraData)
    ...
    elseif RunService:IsStudio() then
        warn(string.format(
[[Speaker '%s' is not in channel '%s' and cannot be sent a system message]], speakerName, self.Name))
    end
end
...
function methods:SendMessageToSpeaker(
    message,
    speakerName,
    fromSpeakerName,
    extraData
    ...
    if speakerTo and speakerFrom then
        ...
        local filterSuccess, isFilterResult, filteredMessage = self.ChatService:InternalApplyRobloxFilterNewAPI(messageObj.FromSpeaker, message, textContext)
    ...
    elseif RunService:IsStudio() then
        warn(string.format(
[[Speaker '%s' is not in channel '%s' and cannot be sent a message]], speakerName, self.Name))
    end
end
...
function methods:InternalPostMessage(fromSpeaker, message, extraData)
    ...
    local filterSuccess, isFilterResult, filteredMessage = self.ChatService:InternalApplyRobloxFilterNewAPI(messageObj.FromSpeaker, message, textFilterContext)
...
function methods:InternalCreateMessageObject(
    message,
    fromSpeaker,
    isFiltered,
    extraData
...
function methods:RegisterGetWelcomeMessageFunction(func)
    if type(func) ~= 'function' then
        error(
[[RegisterGetWelcomeMessageFunction must be called with a function.]])
    end
