...
local function Run(ChatService)
    ...
    local function DoWhisperCommand(fromSpeaker, message, channel)
        ...
        elseif otherSpeakerError == DisplayNameHelpers.CommandErrorCodes.NoMatches then
            local msg = ChatLocalization:FormatMessageToSend('GameChat_MuteSpeaker_SpeakerDoesNotExist', string.format("Speaker '%s' does not exist.", tostring(otherSpeakerInputName)), 'RBX_NAME', tostring(otherSpeakerName))
    ...
    local function PrivateMessageReplicationFunction(
        fromSpeaker,
        message,
        channelName
    )
    ...
    local function PrivateMessageAddTypeFunction(
        speakerName,
        messageObj,
        channelName
    )
    ...
    ChatService.SpeakerAdded:connect(function(speakerName)
        ...
        channel.WelcomeMessage = ChatLocalization:FormatMessageToSend('GameChat_PrivateMessaging_NowChattingWith', 'You are now privately chatting with ' .. speakerDisplayName .. '.', 'RBX_NAME', tostring(speakerDisplayName))
