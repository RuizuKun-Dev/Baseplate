    ...
    local function ProcessCommandsFunction(fromSpeaker, message, channel)
        if (message:lower() == '/?' or message:lower() == '/help') then
            ...
            if ShowJoinAndLeaveCommands() then
                ...
                speaker:SendSystemMessage(ChatLocalization:FormatMessageToSend('GameChat_ChatCommandsTeller_LeaveChannelCommand', 
[[/leave <channel> or /l <channel> : leave channel. (leaves current if none specified)]]), channel)
            end

            speaker:SendSystemMessage(ChatLocalization:FormatMessageToSend('GameChat_ChatCommandsTeller_WhisperCommand', 
[[/whisper <speaker> or /w <speaker> : open private message channel with speaker.]]), channel)
            ...
            if player and player.Team then
                speaker:SendSystemMessage(ChatLocalization:FormatMessageToSend('GameChat_ChatCommandsTeller_TeamCommand', 
[[/team <message> or /t <message> : send a team chat to players on your team.]]), channel)
            end
