...
local function Run(ChatService)
    ...
    channel.WelcomeMessage = ChatLocalization:FormatMessageToSend('GameChat_TeamChat_WelcomeMessage', 
[[This is a private channel between you and your team members.]])
    ...
    local function TeamChatReplicationFunction(
        fromSpeaker,
        message,
        channelName
    )
        ...
        local changedConn = player.Changed:connect(function(property)
            ...
            if speakerObj then
                ...
                elseif property == 'Team' then
                    ...
                    if speakerObj:IsInChannel(channel.Name) then
                        local msg = ChatLocalization:FormatMessageToSend('GameChat_TeamChat_NowInTeam', string.format("You are now on the '%s' team.", player.Team.Name), 'RBX_NAME', player.Team.Name)
