...
function methods:DoLockChatBar()
    if self.TextLabel then
        if LocalPlayer.UserId > 0 then
            self.TextLabel.Text = ChatLocalization:Get('GameChat_ChatMessageValidator_SettingsError', 'To chat in game, turn on chat in your Privacy Settings.')
        else
...
function methods:SetUpTextBoxEvents(TextBox, TextLabel, MessageModeTextButton)
    ...
    self.TextBoxConnections.UserInputBegan = UserInputService.InputBegan:connect(function(
        inputObj,
        gpe
    )
    ...
    self.TextBoxConnections.TextBoxChanged = TextBox.Changed:connect(function(
        prop
    )
    ...
    self.TextBoxConnections.TextBoxFocusLost = TextBox.FocusLost:connect(function(
        enterPressed,
        inputObject
    )
        self:CalculateSize()
...
local function measureSize(textObj)
    return TextService:GetTextSize(textObj.Text, textObj.TextSize, textObj.Font, Vector2.new(textObj.AbsoluteSize.X, 10000))
end
...
function methods:TweenToTargetYSize()
    ...
    local tweeningTime = math.min(1, (pixelDistance * (1 / self.TweenPixelsPerSecond)))
...
function methods:ResetCustomState()
    if self.InCustomState then
        ...
        self:SetTextLabelText(ChatLocalization:Get('GameChat_ChatMain_ChatBarText', 'To chat click here or press "/" key'))
    end
end
...
function methods:Update(dtScale)
    self.AnimParams.Text_CurrentTransparency = CurveUtil:Expt(self.AnimParams.Text_CurrentTransparency, self.AnimParams.Text_TargetTransparency, self.AnimParams.Text_NormalizedExptValue, dtScale)
    self.AnimParams.Background_CurrentTransparency = CurveUtil:Expt(self.AnimParams.Background_CurrentTransparency, self.AnimParams.Background_TargetTransparency, self.AnimParams.Background_NormalizedExptValue, dtScale)
