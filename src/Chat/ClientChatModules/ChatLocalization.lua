...
local ChatLocalization = {_hasFetchedLocalization = false}
...
function ChatLocalization:_getTranslator()
    if not self._translator and not self._hasFetchedLocalization then
        ...
        if localizationTable then
            ...
            warn(
[[Missing ChatLocalization. Chat interface will not be localized.]])
        end
    end
...
function ChatLocalization:FormatMessageToSend(
    key,
    defaultMessage,
    parameterName,
    value
    ...
    if userShouldLocalizeServerMessages then
        if parameterName and value then
            return'{RBX_LOCALIZATION_KEY}' .. key .. '{RBX_LOCALIZATION_DEFAULT}' .. defaultMessage .. '{RBX_LOCALIZATION_PARAMS}' .. parameterName .. '=' .. value
        else
